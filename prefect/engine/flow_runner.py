...
class FlowRunner(Runner):
    ...
    def log_publisher(self, message, level='debug'):
        ...
        if self.flow.publisher:
            self.flow.publisher.publish(message, True)
    ...
    def call_runner_target_handlers(self, old_state: State, new_state: State) -> State:
        ...
        Returns:
            - State: the new state
        """
        self.log_publisher("Flow '{name}': Handling state change from {old} to {new}".format(
            name=self.flow.name,
    ...
    def run(
        ...
        self.log_publisher("Beginning Flow run for '{}'".format(self.flow.name), 'info')
        ...
        try:
            ...
            self.log_publisher("Interrupt signal raised, cancelling Flow run.")
        ...
        # All other exceptions are trapped and turned into Failed states
            ...
            self.log_publisher("Unexpected error while running flow: {}".format(
                repr(exc)), 'exception')
    ...
    def check_flow_reached_start_time(self, state: State) -> State:
        ...
        if isinstance(state, Scheduled):
            if state.start_time and state.start_time > pendulum.now("utc"):
                self.log_publisher("Flow '{name}': start_time has not been reached; ending run.".format(
                    name=self.flow.name))
                raise ENDRUN(state)
        return state
    ...
    def check_flow_is_pending_or_running(self, state: State) -> State:
        ...
        if state.is_finished() is True:
            self.log_publisher("Flow run has already finished.", 'info')
            raise ENDRUN(state)
        ...
        # the flow run must be either pending or running (possibly redundant with above)
            ...
            self.log_publisher("Flow is not ready to run.", 'info')
            raise ENDRUN(state)
    ...
    def set_flow_to_running(self, state: State) -> State:
        ...
        """
        if state.is_pending():
            self.log_publisher("Starting flow run.", 'info')
    ...
    def get_flow_run_state(
        ...
        if not state.is_running():
            self.log_publisher("Flow is not in a Running state.", 'info')
            raise ENDRUN(state)
    ...
    def determine_final_state(
        ...
        if not all(s.is_finished() for s in terminal_states):
            self.log_publisher("Flow run RUNNING: terminal tasks are incomplete.", 'info')
            state.result = return_states

        # check if any key task failed
        elif any(s.is_failed() for s in key_states):
            self.log_publisher("Flow run FAILED: some reference tasks failed.", 'info')
        ...
        # check if all reference tasks succeeded
            ...
            self.log_publisher("Flow run SUCCESS: all reference tasks succeeded", 'info')
            state = Success(
        ...
        # check for any unanticipated state that is finished but neither success nor failed
            ...
            self.log_publisher("Flow run SUCCESS: no reference tasks failed", 'info')
